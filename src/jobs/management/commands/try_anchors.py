...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        model_fn = lambda x: model.predict(x) - 1

        # show plot
        idx = 0
        np.random.seed(1)
        print('Prediction: ', explainer.class_names[model_fn(test_df.drop(['trace_id', 'label'], 1).as_matrix()[idx].reshape(1, -1))[0]])
        exp = explainer.explain_instance(test_df.drop(['trace_id', 'label'], 1).as_matrix()[idx], model_fn, threshold=0.95, desired_label=True)
